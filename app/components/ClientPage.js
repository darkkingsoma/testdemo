...
export default function ClientPage({ session: serverSession }) {
    ...
    try {
      switch (sectionId) {
        ...
        case 'top-rated-movies':
          endpoint = genreId
            ? `discover/movie?sort_by=vote_average.desc&vote_count.gte=1000&page=${state.page}`
            : `movie/top_rated?page=${state.page}`;
          break;
        default:
          return;
      }
  ...
  const handleGenreChange = (genre) => {
    ...
    console.log('Selected genre:', genre, 'Normalized:', normalizedGenre, 'Genre ID:', genreId);
    ...
    const fetchPromises = ['popular-movies', 'upcoming-movies', 'top-rated-movies'].map(section => {
      console.log(`Fetching ${section} with genre ID:`, genreId);
